...
def _resolve_deck(session: Session, deck_id: int | None):
    """确定目标 Deck；未指定时用 Default Deck 并附带提示"""
    if deck_id:
    ...
    return deck, f"未指定 Deck，已自动分配到 Default Deck（id={deck.id}）"
...
def preview_text(
    ...
    """
    拆解预览：调 LLM 生成卡片候选但不入库，供用户编辑/剔除后再确认导入。

    Raises:
        ValueError: Deck 不存在
        RuntimeError: LLM 拆解失败
    """
    deck, warning = _resolve_deck(session, req.deck_id)

    cards = parse_text_with_llm(req.text, deck.parser_config)
    if not cards:
    ...
    return SourcePreviewResponse(
        title=title,
        deck_id=deck.id,
        cards=[CardCandidate(**c) for c in cards],
        warning=warning,
    )
...
def import_text(
    ...
    2. 取卡片：req.cards（预览确认过的）直接用；否则调 LLM 拆解
    3. 创建 Source + Block
    4. 统一 commit

    Raises:
        ValueError: Deck 不存在
        RuntimeError: LLM 拆解失败 / 确认列表为空
    """
    deck, warning = _resolve_deck(session, req.deck_id)

    if req.cards is not None:
        # 预览确认流：用户已编辑/筛选过，跳过 LLM
        blocks_data = [{"content": c.content.strip(), "quiz": c.quiz.strip()} for c in req.cards]
        blocks_data = [c for c in blocks_data if c["content"] and c["quiz"]]
        if not blocks_data:
            raise RuntimeError("确认的卡片列表为空")
    else:
        blocks_data = parse_text_with_llm(req.text, deck.parser_config)
        if not blocks_data:
            raise RuntimeError("AI 提取失败：未能从文本中生成有效卡片")
